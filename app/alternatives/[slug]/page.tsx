...
export async function generateStaticParams() {
  return getAllMDXFiles('alternatives').map((file) => ({ slug: file.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const file = getMDXFile('alternatives', params.slug);
  if (!file) return {};

  const url = `${SITE_URL}/alternatives/${file.slug}`;
  return {
    title: `${file.frontmatter.title} | The AI Prism`,
    description:
      file.frontmatter.excerpt ||
      `Best alternatives to ${file.frontmatter.title} — tested by The AI Prism.`,
...
export default async function AlternativesSlugPage({ params }: PageProps) {
  const file = getMDXFile('alternatives', params.slug);
  if (!file) notFound();
